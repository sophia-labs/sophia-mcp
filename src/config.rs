...
pub struct Cli {
    ...
    /// scoped `sph_ak1_` API key, Cognito JWT, dev token, or shared service token (`PN_SERVICE_TOKEN`
    ...
    /// name is never treated as globally unique. API keys may instead use a
    /// complete owner-qualified MCP URL without --owner or --graph.
